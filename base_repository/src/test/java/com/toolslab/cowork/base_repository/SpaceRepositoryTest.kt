...
class SpaceRepositoryTest {
    ...
    private val mockSpaceModelConverter: SpaceModelConverter = mock()
    ...
    fun setUp() {
        ...
        underTest.spaceModelConverter = mockSpaceModelConverter
    }

    @Test
    fun listSpaces() {
        val repositorySpace1 = com.toolslab.cowork.base_repository.model.Space("repositorySpace1", "snippet1", 1.1, 1.1)
        val repositorySpace2 = com.toolslab.cowork.base_repository.model.Space("repositorySpace2", "snippet2", 2.2, 2.2)
        whenever(mockCoworkingMapApi.listSpaces(country, city, space)).thenReturn(Single.just(spaces))
        whenever(mockSpaceModelConverter.convert(space1)).thenReturn(repositorySpace1)
        whenever(mockSpaceModelConverter.convert(space2)).thenReturn(repositorySpace2)
